...
上例中，`implements` 后面是类 `Car`，这时 TypeScript 就把 `Car` 视为一个接口，要求 `MyCar` 实现 `Car` 里面的每一个属性和方法，否则就会报错。所以，这时不能因为 `Car` `类已经实现过一次，而在MyCar` 类省略属性或方法。
...
上例中，静态属性 `a` 的返回值类型是 `this`，就报错了。原因是this类型表示实例对象，但是静态成员拿不到实例对象。
